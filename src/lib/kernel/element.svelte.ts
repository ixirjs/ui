...
} from '$ixirjs/ui/kernel/resolve/presentation-props';
...
import type { PlainAttrs } from './render/element-branches.svelte';
...
export type KernelElement = {
	...
	tag(): string;
	class(): string;
	/** Every attribute the plain leaves write, in one call. See `PlainAttrs`. */
...
function valuesFrom(
	...
	return {
		...
		restProps: restPropsFor(seam, current, elementAttributes)
	};
}
...
function buildKernelElement(
	...
	if (declared.oninit !== undefined || hasMintedLifecycleKeys()) {
		runLifecycle(
			() => restPropsFor(seam, props(), attrs()),
			() => (props().bond as Bond | undefined) ?? seam.bond,
			() => props().oninit as never
		);
	}

	// One tracked snapshot over `valuesFrom(props())`, on both platforms — the server evaluates it
	// once, the browser holds it in a single `$derived.by`. The eleven per-axis thunks this used to
	// hand `createPresentation` existed so a derived could re-read each axis; `props` is already the
	// one tracked value they all read, so they were eleven closures, eleven calls and a six-accessor
	// view object per rendered part for nothing.
	const registry = presentationRegistry();
	const snapshot = createSnapshot(() =>
		resolvePresentation(valuesFrom(seam, props(), attrs()), registry)
	);

	/**
	 * One object per presentation, not per read — and its OWN derived, deliberately. When the fold
	 * returns the rest props by reference, spreading them here reads every key inside THIS signal
	 * and not the snapshot's, which is what lets an unrelated attribute change update the element
	 * without re-resolving preset, variants or classes (`resolve-count.svelte.spec.ts` pins it).
	 * Folding the spread into the snapshot was tried this round and re-resolved on every change.
	 */
	const spread = !BROWSER
		? () => ({ class: withDefaultBorder(toClassValue(snapshot().class)), ...snapshot().attrs })
		: (() => {
				const memo = $derived({
					class: withDefaultBorder(toClassValue(snapshot().class)),
					...snapshot().attrs
				});
				return () => memo;
			})();
	...
	const motion: ElementMotion<any> | undefined = declaresTransition(declared)
		? // eslint-disable-next-line @typescript-eslint/no-explicit-any
			useElementMotion<any>({
				motion: () => snapshot().motion as never,
				onmount: () => snapshot().attrs.onmount as never,
				ondestroy: () => snapshot().attrs.ondestroy as never,
				onintroend: () => snapshot().attrs.onintroend as never,
				onexitend: () => snapshot().attrs.onexitend as never,
				// Matches `html-element.svelte`, the renderer this path replaces.
				once: true
			})
		: undefined;

	const modeOf = (config: KernelElementProps): RenderMode => {
		const as = snapshot().as;
		return renderMode({
			...
			// cannot arise here.
			plain: snapshot().attrs === EMPTY_ATTRS,
			motion: snapshot().motion,
			attrs: snapshot().attrs,
			base: config.base ?? snapshot().base,
	...
	return {
		...
		mode(): RenderMode {
			...
			//
			// The config is read TRACKED and the snapshot UNTRACKED, and the split is load-bearing.
			// This runs inside the `{@render Kernel.render(el)}` dispatch block, and Svelte re-runs a
			// block effect eagerly — no `is_dirty`, no equality gate — whenever a source it depends on
			// through any derived chain is written during effect flush. A registration is exactly that
			// write (`onmount` → `Collection.set`), and the snapshot reaches Bond state through the
			// Atom's attrs. Tracking it made every accordion header's dispatch re-run once per item
			// mounted after it: 1206 renders per item at n=800, `bench:growth` k = 1.34. The mode's
			// inputs are the consumer's props and the preset, so the config is the dependency that
			// matters; a snapshot read that is not registered still returns the current value.
			// perf-vs-shadcn-2026-08.md §15.
			const config = props();
			return untrack(() => modeOf(config));
		},
		tag: () => String(snapshot().as ?? 'div'),
		class: () => withDefaultBorder(toClassValue(snapshot().class)),
		// The plain leaves' one-call attribute packet. An element reaching them has `EMPTY_ATTRS`
		// by definition (that identity IS how `mode()` selects the lane), so the class is the only
		// thing it can contribute — but the three absent keys are still spelled, so a plain leaf
		// sees one object shape whether a node or an element handed it over.
		plainAttrs: () => ({
			class: withDefaultBorder(toClassValue(snapshot().class)),
			id: undefined,
			role: undefined,
		...
		spread: () => spread(),
		motion: () => motion,
		resolvedMotion: () => snapshot().motion,
		renderer() {
			const target = resolveRenderTarget(snapshot().base, HtmlElement);
