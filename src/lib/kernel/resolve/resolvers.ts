...
import type { Variants } from '$ixirjs/ui/kernel/types';
import * as utils from './index';
import { simpleRecord } from './default-record';
...
export function resolvePreset(
	...
	if (typeof presetKey === 'string') {
		...
		return entry ? resolveEntry(entry, presetKey, bond) : undefined;
	}
	for (const key of (presetKey as FallbackPreset).presets) {
		const entry = getPreset(key);
		if (entry) return resolveEntry(entry, key, bond);
	}
	return undefined;
}
...
function resolveEntry(
	...
	// The library's own default entries are class-only factories; resolve each once, not per part.
	return (
		simpleRecord(entry, key) ??
		(utils.resolvePreset(entry({ bond })) as PresetEntryRecord | undefined)
	);
}
...
export function resolvePresetLayer(
	...
	// An instance layer has no preset key, so nothing to cache against.
	if (typeof layer === 'function') {
		return utils.resolvePreset(layer({ bond })) as PresetEntryRecord | undefined;
	}
