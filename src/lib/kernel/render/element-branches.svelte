<script module lang="ts">
	...
	export type ElementView = {
		 ...
		 */
		class?(): string;
		/** Every attribute the plain leaves write, in ONE call — see {@link PlainAttrs}. */
		plainAttrs?(): PlainAttrs;
	};

	/**
	 * The complete attribute set of a class-only element, resolved in one call.
	 *
	 * One call rather than four accessors, because of what the compiler does with the difference.
	 * Svelte hands each dynamic attribute expression to `template_effect` as its own entry, and
	 * `flatten` wraps every entry in its own `derived` — so the four accessors this replaced cost
	 * **four signals per rendered element**, of which three only ever read a field. Reading one
	 * `{@const}` instead lets the compiler hoist all four writes into the effect body and emit no
	 * sync array at all: **one derived, whatever the attribute count**. Measured on a card mount,
	 * n=800, nine interleaved rounds: **−9.4%**.
	 *
	 * The shape is fixed rather than DEV-conditional. `bond` and `kind` are `undefined` in
	 * production — a literal attribute whose value is `undefined` emits nothing, which is what lets
	 * one leaf serve both modes — but they are always PRESENT as keys, so every plain element hands
	 * the leaf the same hidden class.
	 */
	export type PlainAttrs = {
		class: string;
		/** A node's element id. `undefined` on `KernelElement`, which carries its id inside `attrs`. */
		id: string | undefined;
		/** A constant ARIA role declared by the plan — see `PlanOptions.role`. */
