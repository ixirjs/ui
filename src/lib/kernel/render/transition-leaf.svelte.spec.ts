...
describe.each<LeafHost>(['bonded', 'static'])('transition leaves via %s host', (host) => {
	 ...
	 */
	// The leaf is decided once at init (2026-08-26: every part binds `Kernel.render(el)` once, which
	// compiles to a direct call with no block and no anchor). A motion removed after init no longer
	// swaps the element away, so the release this used to observe on a mode flip is now observed on
	// the element leaving.
	it('releases the node when the element leaves', async () => {
		const ondestroy = vi.fn();
		const { rerender } = render(Probe, { host, open: true, withMotion: true, ondestroy });
		await settle();

		await rerender({ host, open: false, withMotion: true, ondestroy });
		await settle(200);
