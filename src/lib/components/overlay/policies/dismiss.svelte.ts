...
export const OUTSIDE_PRESS = sharedCapabilityKey<OutsidePressSurface>('@ixirjs/cap:outside-press');
...
export function outsidePressPolicy(
	...
	return defineCapability<OutsidePressSurface>({
		slot: OUTSIDE_PRESS,
		surface,
		meta: {
			projects: [],
...
export function backdropPressPolicy(
	...
	return defineCapability<BackdropPressHandler>({
		slot: BACKDROP_PRESS,
		surface,
		meta: {
			projects: ['backdrop'],
